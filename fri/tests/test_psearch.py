...
def test_psearch(problem, C, iter_psearch):
    ...
    if problem is "regression":
        ...
        fri = FRIRegression(random_state=randomstate, verbose=1, C=C, iter_psearch=iter_psearch, optimum_deviation=0.01)
    elif problem is "classification":
        gen = genClassificationData
        fri = FRIClassification(random_state=randomstate, verbose=1, C=C, iter_psearch=iter_psearch,
                                optimum_deviation=0.01)
    elif problem is "ordreg":
        gen = genOrdinalRegressionData
        fri = FRIOrdinalRegression(random_state=randomstate, verbose=1, C=C, iter_psearch=iter_psearch,
                                   optimum_deviation=0.01)
