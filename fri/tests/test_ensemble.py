...
from fri import (FRIClassification, genData,EnsembleFRI)
...
def test_strongRelevant():
    ...
    rbc = EnsembleFRI(FRIClassification(random_state=generator),n_bootstraps=5, random_state=generator)
    rbc.fit(X, y)
...
def test_multiprocessing():
    ...
    # Test using the score function
    rbc = EnsembleFRI(FRIClassification(random_state=generator),n_bootstraps=5,n_jobs=2, random_state=generator)
    rbc.fit(X, y)
    # non-regression test for missing worst feature:
    assert_equal(len(rbc.allrel_prediction_), X.shape[1])
    assert_equal(len(rbc.interval_), X.shape[1])
