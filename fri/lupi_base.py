...
from .lupi_bounds import LowerBound_lupi, UpperBound_lupi, ShadowLowerBound_lupi, ShadowUpperBound_lupi
...
class FRIBase_lupi(BaseEstimator, SelectorMixin):
    ...
    @abstractmethod
    def __init__(self, isRegression=False, C=None, optimum_deviation=0.001, optimum_deviation_priv=0.001, optimum_deviation_slack=0.001,
                 random_state=None, parallel=False, n_resampling=3, debug=False):
    ...
    def fit(self, X, X_priv, y):
        ...
        self._initEstimator(X, X_priv, y)
        if self.debug:
        ...
        # Calculate bounds
        rangevector, omegas, biase, shadowrangevector = self._main_opt(X, X_priv, y, self.optim_loss_,
                                                                       self.optim_L1_,
                                                                       self.optim_L1_priv_,
                                                                       self.random_state,
    ...
    def _main_opt(self, X, X_priv, Y, svmloss, L1, L1_priv, random_state, shadow_features, presetModel=None, solverargs=None):
        ...
        #
        work = [LowerBound_lupi(problemClass=self, optim_dim=di, kwargs=kwargs, initLoss=svmloss, initL1=L1, initL1_priv=L1_priv, X=X, X_priv=X_priv, Y=Y,
                           presetModel=presetModel)
                for di in dims]
        work.extend([UpperBound_lupi(problemClass=self, optim_dim=di, kwargs=kwargs, initLoss=svmloss, initL1=L1, initL1_priv=L1_priv, X=X, X_priv=X_priv, Y=Y,
                                presetModel=presetModel)
                     for di in dims])
        if shadow_features:
            for nr in range(self.n_resampling):
                work.extend([ShadowLowerBound_lupi(problemClass=self, optim_dim=di, kwargs=kwargs, initLoss=svmloss,
                                              initL1=L1, initL1_priv=L1_priv, X=X, X_priv=X_priv, Y=Y, sampleNum=nr, presetModel=presetModel)
                             for di in dims])
                work.extend([ShadowUpperBound_lupi(problemClass=self, optim_dim=di, kwargs=kwargs, initLoss=svmloss,
                                              initL1=L1, initL1_priv=L1_priv, X=X, X_priv=X_priv, Y=Y, sampleNum=nr, presetModel=presetModel)
                             for di in dims])
    ...
    def _initEstimator(self, X, X_priv, Y):
